...
//  DrawDemo
//
//  Created by yanghuan on 2018/11/3.
...
-(void)drawRect:(CGRect)rect{
    ...
    if (_atIndex == 0) {
        ...
        [self drawRectByUIKitWithContext2];
    }else if (_atIndex == 13){
...
#pragma mark - 重写drawRect:方法绘制两条直线
- (void)drawViewBase{
    //1.取得图形上下文对象
    ...
    CGPathAddLineToPoint(path, nil, 300, 100);//绘制另一条直线（从上一直线终点开始绘制）
    //3.添加路径到图形上下文
...
#pragma mark - 简化绘图方式
//其实Core Graphics 内部对创建对象添加到上下文这两步操作进行了封装
//UIKit内部其实封装了一些以“UI”开头的方法用来进行图形绘制
- (void)drawViewSimp{
    //1.获得图形上下文
    CGContextRef context=UIGraphicsGetCurrentContext();
