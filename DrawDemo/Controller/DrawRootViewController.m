...
@interface DrawRootViewController ()<UINavigationControllerDelegate,UIImagePickerControllerDelegate>
{
...
- (void)viewDidLoad {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (weakSelf.atIndex == 18) {
            ...
            return ;
        }else if (weakSelf.atIndex == 19){
            [weakSelf drawContentToPdfContext];
            return;
        }else if (weakSelf.atIndex == 20){
            [weakSelf initLayout];
            return;
        }
        
        DrawView *drawView = [[DrawView alloc]initWithFrame:self.view.bounds];
        drawView.atIndex = weakSelf.atIndex;
        drawView.backgroundColor = [UIColor whiteColor];
        [weakSelf.view addSubview:drawView];
    });
...
-(UIImage *)drawImageAtImageContext{
    ...
    UIImage *image=[UIImage imageNamed:@"lili.png"];
...
-(void)drawContentToPdfContext{
    ...
    UIImage *image=[UIImage imageNamed:@"test01.png"];
    [image drawInRect:CGRectMake(316, 20, 290, 305)];
    
    UIImage *image2=[UIImage imageNamed:@"test02.png"];
    [image2 drawInRect:CGRectMake(6, 320, 600, 281)];
    
    //创建新的一页继续绘制其他内容
    UIGraphicsBeginPDFPage();
    UIImage *image3=[UIImage imageNamed:@"lili.png"];
...
-(void)initLayout{
    ...
    lbSaturation.text=@"饱和度"; //Saturation
    ...
    lbBrightness.text=@"亮度"; //Brightness
